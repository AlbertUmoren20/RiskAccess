...
const ERMPage = () => {
  ...
  standard: 'erm',
    ...
    const {data, error } = await supabase.from('tasks').insert([{
      ...
      standard: 'erm', // or 'pci', etc. — hardcode or dynamically set
      assigned_to: selectedMembers.join(', '),
      status: 'Not Started'
    }]).select();
...
useEffect(() => {
  ...
  return (
    <div className="Page">
      <h1 className="title">Enterprise Risk Mgt. PAGE</h1>
      <p className="description">This is the standard task page for ERM</p>
            ...
            {events.map((event) => (
              <div className="task-item" key={event.id}>
                <div className="task-info">
                  <div className="task-name">{event.title}</div>
