...
const PCIPage = () => {
  const { standardsEvents, updateStandardEvents } = useStandards();
  ...
  const handleAddEvent = async () => {
    try {
      ...
      const updatedEvents = [...localEvents, newEvent];
            ...
            updateStandardEvents('pci', updatedEvents);
  ...
  const fetchEvents = async () => {
    try {
      ...
      updateStandardEvents('pci', data.items || []);
    } catch (err) {
      ...
      if (confirm) {
        try {
          ...
          await fetch(
           ...
           setLocalEvents((prev) => {
            const updatedEvents = prev.filter((event) => event.id !== eventToDelete);
            setEvents(updatedEvents); // Sync with context
  ...
  return (
    <div className="Page">
      ...
      <h2 className="section-title" onClick={toggleDropdown}>QMS Tasks</h2>
